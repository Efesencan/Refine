...
def main():
    ...
    eclipse_result_df = pd.DataFrame(
        columns=[
            "train_anomaly_ratio",
            "test_anomaly_ratio",
            "dataset",
    ...
    actual_train_data_labels = actual_train_data_label["anom"]

    ### iterative robust VAE
    iterations = 30  # Number of iterations to repeat the process
    epoch_count = 400  # Number of epochs for initial training
    alpha = 2  # Multiplier for standard deviation in error threshold

    prev_reconstruction_errors = None
    prev_error_threshold = None

    # Initial training data and labels
    train_data = actual_train_data.copy()
    train_data_label = actual_train_data_label.copy()
    train_data_labels = actual_train_data_label["anom"].copy()

    for i in range(iterations):
        print(f"Iteration {i+1}/{iterations}")
        vae_model = create_vae_model(train_data)

        # Define the callbacks
        # reconstruction_error_callback = ReconstructionErrorCallback(train_data, train_data_label, n_epochs=10)
        plot_error_callback = PlotReconstructionErrorCallback(
            train_data, train_data_labels, n_epochs=5
        )
        checkpoint_path = f"/projectnb/peaclab-mon/sencan/vae_model_checkpoints/{SYSTEM}/{FE_NAME}/vae_model_{str(epoch_count)}_PCA_checkpoint_iteration_{i+1}.h5"
        ...
        plot_average_samples_errors_callback = PlotAverageSamplesErrorCallback(
            train_data, train_data_labels, n_epochs=20
        )
        ...
        results = vae_model.fit(
            ...
            batch_size=256,
            # validation_data=(healthy_validation_data if train_with_only_healthy_data else validation_data, healthy_validation_data if train_with_only_healthy_data else validation_data),
            callbacks=[plot_average_samples_errors_callback, checkpoint],
        )  # previosuly it was plot_error_callback, checkpoint

        errors = np.array(plot_average_samples_errors_callback.avg_reconstruction_errors)
        epochs = np.array([i for i in range(1, 401)])

        # Find the knee point
        knee_locator = KneeLocator(epochs, errors, curve="convex", direction="decreasing")
        ...
        results = vae_model.fit(
            ...
            train_data,
            shuffle=True,
            epochs=knee_point,  # instead if fix 30 epoch_count
            batch_size=256,
            # validation_data=(healthy_validation_data if train_with_only_healthy_data else validation_data, healthy_validation_data if train_with_only_healthy_data else validation_data),
            callbacks=[checkpoint],
        )  # previosuly it was plot_error_callback, checkpoint

        # Calculate reconstruction errors and filter the training data
        reconstructed = vae_model.predict(train_data)
        reconstruction_errors = np.mean(np.square(train_data - reconstructed), axis=1)
        mean_error, std_error = np.mean(reconstruction_errors), np.std(reconstruction_errors)
        error_threshold = mean_error + alpha * std_error
        samples_to_exclude = reconstruction_errors > error_threshold
        filtered_train_data = train_data[~samples_to_exclude]

        # Update train data and labels for next iteration
        filtered_train_data_labels = train_data_label[~samples_to_exclude]

        if prev_reconstruction_errors is not None:
            wasserstein_dist = wasserstein_distance(
                prev_reconstruction_errors, reconstruction_errors
            )
        else:
            wasserstein_dist = np.inf

        # Create new model for the next iteration
        print("Create new model from scratch")
        new_vae_model = create_vae_model(filtered_train_data)

        # Redefine the callbacks
        plot_error_callback = PlotReconstructionErrorCallback(
            filtered_train_data, filtered_train_data_labels["anom"], n_epochs=20
        )
        reconstruction_error_callback = ReconstructionErrorCallback(
            filtered_train_data, filtered_train_data_labels, n_epochs=20
        )
        checkpoint_path = f"/projectnb/peaclab-mon/sencan/vae_model_checkpoints/{SYSTEM}/{FE_NAME}/vae_model_filtered_iteration_{i+1}.h5"
        checkpoint = ModelCheckpoint(
            checkpoint_path,
            save_weights_only=True,
            save_best_only=True,
        ...
        retrain_results = new_vae_model.fit(
            filtered_train_data,
            filtered_train_data,
            epochs=400,
            batch_size=256,
        ...
        # Update previous iteration metrics
        prev_reconstruction_errors = reconstruction_errors
        prev_error_threshold = error_threshold

        # Evaluate the model
        X_train_pred = new_vae_model.predict(filtered_train_data)
        mae_vector = get_error_term(X_train_pred, filtered_train_data, _rmse=False)

        print(
            f"Avg train error {np.mean(mae_vector)}\nmedian train error"
            f" {np.median(mae_vector)}\ntrain 99Q: {np.quantile(mae_vector, 0.99)}"
        )
        print("*********************************")
        ...
        # Test the model
        X_pred = new_vae_model.predict(test_data)
        test_mae_vector = get_error_term(X_pred, test_data, _rmse=False)
        anomalies = test_mae_vector > error_thresh
        y_test = [0.0 if row["anom"] == 0 else 1.0 for index, row in test_label.iterrows()]
        ...
        # Find the set difference based on the 'node_id' index
        difference_node_ids = set(train_data_label.index) - set(filtered_train_data.index)

        # Filter actual_train_data_label based on the node_ids that exist in the set difference
        filtered_actual_data = train_data_label.loc[
            train_data_label.index.isin(difference_node_ids)
        ]

        # Get the distribution of labels in the 'anom' column
        label_distribution = filtered_actual_data["anom"].value_counts()

        filtered_train_data_label = actual_train_data_label.loc[
            actual_train_data_label.index.isin(filtered_train_data.index)
        ]
        ...
        new_row = {
            ...
            "test_anomaly_ratio": 0.93,
            "dataset": SYSTEM,
            "f1-score": classification_results["macro avg"]["f1-score"],
            "cv_fold": CV_INDEX,
            "train_anomaly_count": len(train_data_label)
            - train_data_label["anom"].value_counts()[0],
            "filtered_train_anomaly_count": len(filtered_train_data_label)
            - (filtered_train_data_label["anom"].value_counts()[0]),
            "actual_train_len": len(train_data),
            "filtered_train_len": len(filtered_train_data),
        ...
        eclipse_result_df = eclipse_result_df.append(new_row, ignore_index=True)

        # Update train_data and train_data_labels for the next iteration
        train_data = filtered_train_data
        train_data_label = filtered_train_data_labels
        train_data_labels = filtered_train_data_labels["anom"]

    # Constructing the filename with variable values
    directory_path = (
        "/projectnb/peaclab-mon/sencan/robust_prodigy/rerun_iterative_robust_prodigy_results"
    )

    filename = f"{directory_path}/rerun_stopping_criteria_iterative_robust_prodigy_{str(anomaly_ratio)}%_cv_{CV_INDEX}_{SYSTEM}.csv"

    # Save the dataframe to CSV
    eclipse_result_df.to_csv(filename)
