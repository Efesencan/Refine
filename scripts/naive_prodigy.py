...
def main():
    ...
    vae_model = create_vae_model(actual_train_data)
    ...
    X_train_pred = vae_model.predict(actual_train_data)  # previously it was healthy_train_data
    mae_vector = get_error_term(X_train_pred, actual_train_data, _rmse=False)
    ...
    filename = f"naive_prodigy_{str(anomaly_ratio)}%_cv_{CV_INDEX}_{SYSTEM}.csv"
