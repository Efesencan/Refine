...
def main():
    ...
    # Train LOF on the training data with the training contamination ratio
    lof = LocalOutlierFactor(n_neighbors=20, contamination=anomaly_ratio, novelty=True)
    ...
    # LOF predict method uses -1 for anomalies and 1 for normal, need to invert for AUC calculation
    ...
    filename = f"/projectnb/peaclab-mon/sencan/robust_prodigy/lof_without_test_assumption_results/local_outlier_factor_without_test_assumption_robust_prodigy_{str(anomaly_ratio)}%_cv_{CV_INDEX}_{SYSTEM}.csv"
